...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        #----> Pencerenin adı ve simgesi
        applicationName = "MTY Arabuluculuk Programı"
        self.setWindowTitle(applicationName)
        UIFunction.labelTitle(self, applicationName) #lab_appname ögesine pencere adını kaydet
        UIFunction.initStackTab(self) #Her anasayfaya gelişte çalışması gerekiyor. Sekme ve yazıları varsayılana döndürüyor.
        UIFunction.constantFunction(self) #Çerçeveyi kaldırıyor ve frame_appname'e çift tıklayınca maximize window'u sağlıyor.

        # ----> Sol taraftaki menü butonları
        self.ui.bn_genislet.clicked.connect(lambda: UIFunction.genisletMenu(self, 160, True)) #Sol üstteki genişletme simgesine tıklama fonksiyonu
        self.ui.bn_dosyabilgileri.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_dosyabilgileri'))
        self.ui.bn_taraflar.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_taraflar'))
        self.ui.bn_oturumlar.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_oturumlar'))
        self.ui.bn_kasa.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_kasa'))

        UIFunction.stackPage(self) #Sayfaların buton işlevlerini atar


        # -----> Olası bir durum için dialog ve error box ögelerini hazırlar
        self.diag = dialogUi()
        self.error = errorUi()
        #############################################################
...
#--PAGE BUG----------------------------------:
    ...
    #--USING START BUTTON-----------:
        #self.ui.bn_taraflar_start.clicked.connect(some function to execute) #THE FUNCTION GET ACTIVATED WHEN YOU CLICK THE BUTTON
...
#--PAGE CLOUD--------------------------------:
    ...
    #--USING THE PUSH BUTTONS-------:
        #self.ui.bn_kasa_clear.clicked.connect(function to execute)
        #self.ui.bn_kasa_connect.clikced.connect(function to execute)
...
#--PAGE ANDROID:CONTACT----------------------:
    ...
    #--USING PUSH BUTTONS-----------:
        ...
        #self.ui.bn_oturumlar_contact_save.setEnable(True) #TO ENABLE THE BUTTON
...
#FOR REST OF THE WIDGET GOTO THE Documentation.pdf AND CHECK THE LAYOUT FOR MORE DETAILS.
        ...
        # -----> Pencereyi taşıma fonksiyonunu düzenler ve frame_appname ögesini bu işlev için yetkilendirir
    ...
    # -----> Taşıma fonksiyonu için mouse hareketini takip eder
    def mousePressEvent(self, event):
        self.dragPos = event.globalPos()
